...
import { fetchCompletedOrders, submitOrderReview } from "@/lib/services/firestoreService";
...
import ReviewModal from "@/components/ReviewModal"; // Add import for the new component
...
const Orders = () => {
  ...
  const isPastOrder = (status: OrderDetails["orderStatus"]) => {
  ...
  const handleAddReview = (key: string, order: OrderDetails) => {
    setSelectedOrder({ key, order });
  ...
  const submitReview = async (reviewData: any) => {
    try {
      ...
      if (selectedOrder) {
        // You might want to update UI to show this order has been reviewed
        console.log(`Review submitted for order ${selectedOrder.key}`);
      }
    } catch (error) {
      ...
      <Animated.View style={[styles.orderCard, { opacity: fadeAnim }]}>
        ...
        {isActiveOrder(order.orderStatus) && (
          <TouchableOpacity
            ...
            onPress={() => {
              handleCompletedOrder(key, order);
            }}
          >
            <Text style={styles.trackButtonText}>Mark as Received</Text>
            <MaterialIcons name="arrow-forward" size={16} color="#fff" />
          </TouchableOpacity>
        )}

        {isPastOrder(order.orderStatus) &&
          order.orderStatus === "completed" && (
            <TouchableOpacity
              style={[styles.trackButton]}
              onPress={() => handleAddReview(key, order)}
            >
              <Text style={styles.trackButtonText}>Add a Review</Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      {(isLoading && selectedTab === "active") ||
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>
            Loading your {selectedTab === "active" ? "active" : "past"}{" "}
            orders...
          </Text>
        </View>
      ) : (
      ...
      {selectedOrder && (
        <ReviewModal
          ...
          canteenId={selectedOrder.order.canteenId}
          canteenName={selectedOrder.order.canteenName}
          cart={selectedOrder.order.cart}
          theme={theme}
